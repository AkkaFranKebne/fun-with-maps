...
export const MapPointsList: React.FC<MapPointsListType> = (props) => {
  ...
  const handleOnClick = () => {
    return showMapPointsList
      ? setShowMapPointList(false)
      : setShowMapPointList(true)
  }
  ...
  return (
    <div
      id="map-points-wrapper"
      className={!showMapPointsList ? 'onTop' : undefined}
    >
      <div id="drag-indicator-wrapper" {...swipeHandlers}>
        <div id="drag-indicator" data-testid="drag-indicator" />
      </div>
      <div id="header-wrapper">
        <h2
          role="caption"
          className={!showMapPointsList ? 'undisplayedDesktop' : undefined}
        >
          Check our locations:
        </h2>
        <div
          id="carret-down"
          role="carret-down"
          data-testid="carret-down"
          className={!showMapPointsList ? 'undisplayed' : undefined}
          onClick={handleOnClick}
        />
        <div
          id="carret-up"
          role="carret-up"
          data-testid="carret-up"
          className={showMapPointsList ? 'undisplayed' : undefined}
          onClick={handleOnClick}
        />
        <div
      ...
      <div
        id="map-points-list"
        role="map-points-list"
        className={!showMapPointsList ? 'unvisible' : undefined}
      >
        {list}
      </div>
    </div>
