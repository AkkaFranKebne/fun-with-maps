...
describe('MapPointsList', () => {
  ...
  test('handle on click mobile', async () => {
    ...
    await expect(screen.getByRole('open').classList).not.toContain(
      'undisplayed'
    )
    await expect(screen.getByRole('map-points-list').classList).toContain(
      'unvisible'
    )
    ...
    await expect(screen.getByRole('close').classList).not.toContain(
      'undisplayed'
    )
    await expect(screen.getByRole('map-points-list').classList).not.toContain(
      'unvisible'
    )
  })
  ...
  test('handle on click desktop', async () => {
    ...
    await expect(
      screen.getByRole('carret-down', { hidden: true })
    ).toBeDefined()
    await expect(screen.getByRole('carret-down').classList).toContain(
      'undisplayed'
    )
    await expect(screen.getByRole('carret-up')).toBeDefined()
    await expect(screen.getByRole('carret-up').classList).not.toContain(
      'undisplayed'
    )
    await expect(screen.getByRole('map-points-list').classList).toContain(
      'unvisible'
    ...
    await expect(screen.getByRole('carret-up').classList).toContain(
      'undisplayed'
    )
    await expect(screen.getByRole('carret-down')).toBeDefined()
    await expect(screen.getByRole('carret-down').classList).not.toContain(
      'undisplayed'
    )
    await expect(screen.getByRole('map-points-list').classList).not.toContain(
      'unvisible'
